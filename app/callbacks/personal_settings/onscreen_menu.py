...
@register_update
@chat_language
def visibility_callback(
    update: Update, context: CallbackContext, chat_info: dict, _: gettext
):
    if chat_info["is_show_keyboard"]:
        text_to = _(
            "On-screen menu below with a history requests *always shows* at the moment."
        )
    else:
        text_to = _(
            "On-screen menu below with a history requests *never shows* at the moment."
        )

    text_to += "\n"

    text_to += _(
        "You can choose. Show always on-screen menu below with a history requests or never show."
    )

    update.message.reply_text(
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=ReplyKeyboardMarkup(
            [["1. " + _("Always show")], ["2. " + _("Never show")], ["↩️"]]
        ),
        text=text_to,
    )
...
@register_update
@chat_language
def visibility_set_true_callback(
    update: Update, context: CallbackContext, chat_info: dict, _: gettext
):
...
@register_update
@chat_language
def visibility_set_false_callback(
    update: Update, context: CallbackContext, chat_info: dict, _: gettext
):
...
def edit_history_callback(
    ...
    if have_last_request(update.message.chat_id):
        ...
        update.message.reply_text(
            parse_mode=ParseMode.MARKDOWN, reply_markup=keyboard, text=text_to
        )
...
@register_update
@chat_language
def edit_history_delete_one_callback(
    update: Update, context: CallbackContext, chat_info: dict, _: gettext
):
...
def edit_history_delete_old_callback(
    ...
    subquery = (
        db_session.query(ChatRequests.id)
        .filter_by(chat_id=update.message.chat_id)
        .order_by(ChatRequests.times.desc())
        .limit(size)
    )

    db_session.query(ChatRequests).filter(
        ChatRequests.chat_id == update.message.chat_id, ChatRequests.id.notin_(subquery)
...
@register_update
@chat_language
def edit_history_delete_all_callback(
    update: Update, context: CallbackContext, chat_info: dict, _: gettext
):
    Session().query(ChatRequests).filter_by(chat_id=update.message.chat_id).delete()
    transaction.commit()
...
def size_callback(
    ...
    text_to = (
        _(
            """%(keyboard_size)s - size of on-screen menu with a history requests at the moment.
You can customize the size. *width x height*"""
        )
        % {"keyboard_size": chat_info["keyboard_size"]}
    )
