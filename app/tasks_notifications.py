...
from app.models import Chat, Currency, Notification, NotifyTriggerClauseEnum
...
def is_triggered(
    trigger_clause: NotifyTriggerClauseEnum,
    trigger_value: Decimal,
    last_notification_rate: Decimal,
    ...
    if trigger_clause == NotifyTriggerClauseEnum.more:
        ...
        return (
            abs(last_notification_rate - current_rate)
            / last_notification_rate
            * Decimal("100")
            >= trigger_value
        )
...
def notification_auto_disable(pair: list) -> None:
    ...
    notifications = (
        ...
        .filter_by(is_active=true(), from_currency_id=pair[0], to_currency_id=pair[1])
        .all()
    )
    ...
    for n in notifications:
        ...
        send_notification.delay(
            n.chat_id,
            _(
                "Your notification has been disabled, due to one of the currencies"
    ...
    db_session.query(Notification).filter_by(
        is_active=true(), from_currency_id=pair[0], to_currency_id=pair[1]
...
def send_notification(self, chat_id: int, text: str) -> None:
    ...
    try:
        ...
        db_session.query(Notification).filter_by(
            is_active=true(), chat_id=chat_id
        ).update({"is_active": false()})

        db_session.query(Chat).filter_by(id=chat_id).update({"is_subscribed": false()})
...
def notification_checker() -> None:
    db_session = Session()
    pairs = (
        db_session.query(Notification.from_currency_id, Notification.to_currency_id)
        .filter_by(is_active=true())
        .group_by(Notification.from_currency_id, Notification.to_currency_id)
        .all()
    )
    ...
    for pair in pairs:
        ...
        notifications = (
            db_session.query(Notification)
            .filter_by(is_active=true())
            .filter_by(from_currency_id=pair[0], to_currency_id=pair[1])
            .all()
        )
