...
class Chat(BaseObject):
    ...
    default_currency = sa.Column(
        sa.Text, default=settings.DEFAULT_CURRENCY, nullable=False
    )
    default_currency_position = sa.Column(
        sa.Boolean, default=settings.DEFAULT_CURRENCY_POSITION, nullable=False
    )
...
class ChatRequests(BaseObject):
    ...
    from_currency_id = sa.Column(
        sa.Integer, sa.ForeignKey("currencies.id"), nullable=False
    )
...
class Rate(BaseObject):
    ...
    exchange = orm.relationship(
        "Exchange"
    )  # TODO: on all relations cascade = "all, delete, delete-orphan"
    from_currency = orm.relationship("Currency", foreign_keys=[from_currency_id])
    to_currency = orm.relationship("Currency", foreign_keys=[to_currency_id])
...
class Notification(BaseObject):
    ...
    trigger_value = sa.Column(
        sa.Numeric(decimal_precision, decimal_scale), nullable=False
    )
